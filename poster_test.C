...
void poster_test(){
  ...
  //TH2D *hist_weight = new TH2D("hist_weight","hist_weight",N_cost_bins_BC,value_cost_BC,N_phi_bins_BC,value_phi_BC);
  //hist_weight -> FillRandom("func_width",1000000);
  //hist_weight -> Scale(1/func_width -> GetMaximum());
  ...
  for(int i = 0;i < N_test;i++){
          ...
          if(DimuY_rec[k] > -4. && DimuY_rec[k] < -2.5){
            if(DimuMatch_rec[k] == 2){
              if(DimuMass_rec[k] > 2 && DimuMass_rec[k] < 5){
                ...
                //hist_CostPhiHE_2pt6_VAR_pol_rec[i] -> Fill(CostHE_rec[k],PhiHE_rec[k]);
                count_Dimu_rec++;
              }
            }
          }
    ...
    printf("POLARIZATION = (%2.1f,%2.1f,%2.1f) \n",lambdaTh + 0.2*i,lambdaPhi,lambda_ThPhi);
    event_index = 0;
    count_Dimu_rec = 0;
  }

  //hist_CostPhiHE_2pt6_VAR_pol_rec[0] -> Multiply(hist_weight);

  //TCanvas *c_CostPhiHE_2pt6_VAR_pol_rec = new TCanvas("c_CostPhiHE_2pt6_VAR_pol_rec","c_CostPhiHE_2pt6_VAR_pol_rec",20,20,600,600);
  //c_CostPhiHE_2pt6_VAR_pol_rec -> Divide(2,5);

  //for(int i = 0;i < 1;i++){
    //c_CostPhiHE_2pt6_VAR_pol_rec -> cd(i+1);
    //hist_CostPhiHE_2pt6_VAR_pol_rec[i] -> Draw("COLZ");
  //}
  ...
  for(int i = 0;i < N_test;i++){
    ...
    fit_func_CostPhi2D_HE_VARNO_pol[i] = new TF2(func_name,Func_W,-0.6,0.6,0,PI,4);
  ...
  TCanvas *c_lambdaTh_gen_rec = new TCanvas("c_lambdaTh_gen_rec","c_lambdaTh_gen_rec",20,20,600,600);
